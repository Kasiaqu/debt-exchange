  ...
  const formatValue = (header: Headers, value: string | number) => {
    switch (header) {
      case Headers.Name:
        return value;
      case Headers.NIP:
        return (value as string).replace(/(\d{3})(\d{3})(\d{2})(\d{2})/, '$1-$2-$3-$4');;
      case Headers.Value:
          ...
          <thead className="table__header">
            <tr>
              {headers.map((header, index) => (
                <th key={index} className={`table__header-cell ${alignCell(header.name)}`} onClick={() => sortByColumn(debts, header.name)}>
                  <div className={`table__header-cell-flex`}>
                    ...
                    {sortColumn === header.name &&
                      <img src="arrow.svg" alt="arrow"  className={`table__header-arrow ${sortOrder === 'asc' ? 'arrow-asc' : 'arrow-desc'}`} height="12px" width="auto" />}
                    </div>
                  </div>
                </th>
              ))}
            ...
            ? debts.map((row, rowIndex) => (
              <tr key={rowIndex} className="table__row">
                {Object.entries(row).map(([key, value], colIndex) => (
                  <td key={colIndex} className={`table__cell ${alignCell(key as Headers)}`}>
